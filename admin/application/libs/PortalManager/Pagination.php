...
class Pagination
{
	...
	public function render()
	{
		...
		$r .= '<ul class="'.$this->class.'">';
		  $r .= '<li><a href="'.$this->root.'/1"><i class="fa fa-angle-left"></i></a></li>';
		  if( $this->current_item-1 > ($this->page_limit/2) &&  $this->max_item > $this->page_limit )  {
		  	$r .= '<li ><a href="'.$this->root.'/1">1</a></li>';
		  }
		  for($p = 1; $p <= $this->max_item; $p++):
		  	...
		  	$r .= '<li class="'.( $this->current_item == $p  ? 'active' : '' ).'"><a href="'.$this->root.'/'.$p.'">'.$p.'</a></li>';
		  endfor; 
		  if( ($this->current_item < $this->max_item - ($this->page_limit/2) ) &&  $this->max_item > $this->page_limit )  {
		  	$r .= '<li ><a href="'.$this->root.'/'.$this->max_item.'">'.$this->max_item.'</a></li>';
		  }
		  $r .= '<li><a href="'.$this->root.'/'.$this->max_item.'"><i class="fa fa-angle-right"></i></a></li>';
		$r .= '</ul>';
		$r .= '</div>';
